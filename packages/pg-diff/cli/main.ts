...
function CheckDoubleActionError(lastAction: Actions | null, currentAction: string) {
	if (lastAction) {
		HandleError(
			new Error(
				`Too many execution options specified! "${lastAction}" and "${currentAction}" cannot co-exists.`,
			),
		);
		CLI.PrintHelp();
		process.exit();
...
export async function Run() {
	...
	switch (action) {
		...
		case actions.COMPARE:
			{
				...
				if (scriptFilePath)
					log(
						chalk.whiteBright('SQL patch file has been created succesfully at: ') +
							chalk.green(scriptFilePath),
					);
		...
		case actions.MIGRATE_TO_TARGET:
			{
				...
				if (action == actions.MIGRATE_TO_SOURCE) {
					if (
						!optionParams.has(actions.MIGRATE_TO_SOURCE) ||
						optionParams.get(actions.MIGRATE_TO_SOURCE).length != 1
					) {
				...
				if (action == actions.MIGRATE_TO_TARGET) {
					if (
						!optionParams.has(actions.MIGRATE_TO_TARGET) ||
						optionParams.get(actions.MIGRATE_TO_TARGET).length != 1
					) {
		...
		case actions.GENERATE_CONFIG:
			{
				if (
					!optionParams.has(actions.GENERATE_CONFIG) ||
					optionParams.get(actions.GENERATE_CONFIG).length > 1
				) {
